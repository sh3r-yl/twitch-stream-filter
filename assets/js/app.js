    ...
    headers: {
      ...
      success: function (data) {
        ...
        for (var i = 0; i < streamlist.length; i++){
          $('.game').append('<div class="col"><a href="' + streamlist[i].channel.url + '" target="_blank"><article class="card"><img src="' + streamlist[i].preview.medium + '"><div class="card__content"><h2>' + streamlist[i].channel.status + '</h2><p class="streamer">' + streamlist[i].channel.display_name + '</p><p class="description"><strong>Game: </strong>' + streamlist[i].game + '</p></article></a></div>');
        }
      },
      error: function() {
          alert(search);
